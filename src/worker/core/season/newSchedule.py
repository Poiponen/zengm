from common import WEBSITE_ROOT
from common.types import Conditions
from util import g, helpers, log_event
...
def new_schedule(teams, conditions=None):
    ...
    if trade_deadline < 1:
        index = round(helpers.bound(trade_deadline, 0, 1) * len(tids))
        tids.insert(index, [-3, -3])

    # Add an All-Star Game
    all_star_game = g.get("allStarGame")
    if all_star_game is not None and all_star_game >= 0:
        index = round(helpers.bound(all_star_game, 0, 1) * len(tids))
        tids.insert(index, [-1, -2])
